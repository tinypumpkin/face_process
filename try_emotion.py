...
class face_emotion():
    ...
    #text函数将英文转换成中文此处要感谢PIL库233
    def text(self,img,text,my_x,my_y):
        ...
        draw.text((my_x,my_y),text, (255, 0, 0), font=font) # 参数1：打印坐标，参数2：文本，参数3：字体颜色，参数4：字体
        ...
        while(1):
            ...
            if (len(self.face) > 0):
                ...
                for k,d in enumerate(self.face):
                    ...
                    # 标出68个点的位置
                    for i in range(68):
                        cv2.circle(img,(shape.part(i).x,shape.part(i).y),1,(255,0,0),-1,8)
                    ...
                    if round(mouth_higth >= 0.03):
                        if eye_hight >= 0.05:
                            my_text="惊讶"
                            img=self.text(img,my_text,d.left(),d.bottom())
                        else:
                            my_text="开心"
                            img=self.text(img,my_text,d.left(),d.bottom())
                ...
                cv2.putText(img, "No Face", (20, 50), cv2.LINE_AA, 1, (0, 0, 255), 1, )
